...
class SlackAIAgent {
  ...
  setupExpress() {
    ...
    // analyzeWithAI: It will use Groq's freely available
    // compound-mini model series
    ...
    async analyzeWithAI(memberInfo, researchData) {
        ...
        try {
            ...
            // As LLMs response in string format, json provided by LLM
            // is also in string.

            // Hence, we need to extract the json from the string.
            ...
            return {
                fitScore: Math.max(0, Math.min(100, analysis.fitScore || 50)), // 0 - 100
        ...
        const blocks = [
            ...
            },
            {
                ...
                fields: [
                    ...
                    {
                        type: 'mrkdown',
                        text: `*Email:* ${member.email || 'Not provided'}/100`
                    },
                    {
                        type: 'mrkdown',
                        text: `*Title:* ${member.title || 'Not provided'}/100`
                    }
                ]
            }
        ];
    ...
    async stop() {
        ...
        try {
            ...
            if (this.server) {
                await new Promise.resolve(resolve => this.server.close(resolve));
            }
            await closeDatabase();
